    ...
    switch (subcommand) {
      ...
      case 'shop-sell':
        if (focusedOption.name === 'itemname') {
          ...
          await handleCharacterBasedCommandsAutocomplete(interaction, focusedOption, 'shops');
        }
        break;
      case 'trade':
...
async function handleGift(interaction) {
  ...
  try {
    ...
    for (const { name, quantity } of items) {
      await removeItemInventoryDatabase(fromCharacter._id, name, quantity, interaction);
