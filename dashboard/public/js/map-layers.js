...
class MapLayers {
    ...
    _fogClipPathForQuadrants(quads) {
        ...
        const key = q.join(',');
        // Single polygon per key; for diagonal pairs (1,4) and (2,3) use two polygons to avoid self-intersection
        const polygons = {
        ...
        // Diagonal pairs: use two separate polygons so fill doesn't create a diagonal cutoff
        const twoPolygons = {
            '1,4': 'polygon(0% 0%, 50% 0%, 50% 50%, 0% 50%), polygon(50% 50%, 100% 50%, 100% 100%, 50% 100%)',
            '2,3': 'polygon(50% 0%, 100% 0%, 100% 50%, 50% 50%), polygon(0% 50%, 50% 50%, 50% 100%, 0% 100%)'
        };
