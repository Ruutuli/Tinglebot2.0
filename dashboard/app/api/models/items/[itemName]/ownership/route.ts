...
export async function GET(
  ...
  try {
    ...
    logger.info("ownership route.ts", `Found ${collectionNames.length} collections: ${collectionNames.join(", ")}`);
    ...
    for (const collectionName of collectionNames) {
      try {
        ...
        logger.info("[ownership route.ts]", `Skipped collection "${collectionName}": ${err instanceof Error ? err.message : String(err)}`);
      }
    }
    ...
    logger.info("[ownership route.ts]", `Found ${inventoryData.length} characters with "${itemName}"`);
    ...
    logger.error("[ownership route.ts]❌ Failed to fetch character ownership:", errorMessage);
    if (errorStack) {
      logger.error("[ownership route.ts] Stack trace:", errorStack);
    }
    return NextResponse.json(
