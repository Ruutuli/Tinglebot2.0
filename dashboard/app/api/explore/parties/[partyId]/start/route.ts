...
const DISCORD_EXPEDITION_CHANNEL_ID = "1229245060168810506";
...
export async function POST(
  ...
  try {
    ...
    const embed: {
      ...
      fields: [
        ...
        },
        {
          ...
          value:
            characters.length > 0
              ? `**Take your turn:**\n• </explore move:${await getExploreCommandId()}> — id: \`${partyId}\` charactername: **${String(characters[0].name)}**`
    ...
    const characterNames = characters.map((c) => String(c.name)).join(", ");
    const threadName = (characterNames
      ? `📍 | Expedition ${partyId} | ${characterNames}`
      : `📍 | Expedition ${partyId}`).slice(0, 100);
