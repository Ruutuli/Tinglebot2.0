...
function fogClipPathForQuadrants(quads: number[]): string | null {
  ...
  const polygons: Record<string, string> = {
    ...
    "1,3": "0% 0%, 50% 0%, 50% 100%, 0% 100%",
    "2,4": "50% 0%, 100% 0%, 100% 100%, 50% 100%",
    "3,4": "0% 50%, 100% 50%, 100% 100%, 0% 100%",
    "1,2,3": "0% 0%, 100% 0%, 100% 50%, 50% 50%, 50% 100%, 0% 100%",
    "1,2,4": "0% 0%, 100% 0%, 100% 100%, 50% 100%, 50% 50%, 0% 50%",
    "1,3,4": "0% 0%, 50% 0%, 50% 50%, 100% 50%, 100% 100%, 0% 100%",
    "2,3,4": "50% 0%, 100% 0%, 100% 100%, 0% 100%, 0% 50%, 50% 50%",
  };
  // Diagonal pairs (1,4) and (2,3): single polygon would self-intersect and cause diagonal cutoff; use two polygons
  const twoPolygons: Record<string, string> = {
    "1,4":
      "polygon(0% 0%, 50% 0%, 50% 50%, 0% 50%), polygon(50% 50%, 100% 50%, 100% 100%, 50% 100%)",
    "2,3":
      "polygon(50% 0%, 100% 0%, 100% 50%, 50% 50%), polygon(0% 50%, 50% 50%, 50% 100%, 0% 100%)",
  };
  if (twoPolygons[key]) return twoPolygons[key];
                ...
                {(() => {
                  ...
                  return (
                    <section className="rounded-2xl border border-[var(--totk-dark-ocher)]/50 bg-[var(--botw-warm-black)]/40 p-4 shadow-inner">
                      ...
                      ) && party.quadrantState === "secured" && party.quadrant && wasSecuredThisSession(party.progressLog, party.square, party.quadrant) && (
