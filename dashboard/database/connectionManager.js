...
class DatabaseConnectionManager {
  ...
  static async connectToTinglebot() {
    ...
    connectionPromises.tinglebot = retryOperation(async () => {
      ...
      // Sync TTL indexes (only if models exist)
      try {
        // Try to load models - they may not all exist in dashboard
        const modelPromises = [];
        
        try {
          const TempData = require('../models/TempDataModel');
          modelPromises.push(TempData.syncIndexes());
        } catch (e) {
        ...
        try {
          ...
          modelPromises.push(RuuGame.syncIndexes());
        } catch (e) {
          // Model doesn't exist, skip
        }
        ...
        try {
          ...
          modelPromises.push(VendingRequest.syncIndexes());
        } catch (e) {
          // Model doesn't exist, skip
        }
