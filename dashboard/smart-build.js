...
const dashboardPath = dashboardDir || 'dashboard';
    ...
    try {
      ...
      const dashboardFilesChanged = changedFiles.some(file => {
        ...
        return normalizedFile.startsWith(`${dashboardPath}/`) || 
               normalizedFile.startsWith('dashboard/');
      });

      // Also check if shared files that dashboard depends on changed
      const sharedFilesChanged = changedFiles.some(file => {
        const normalizedFile = file.replace(/\\/g, '/');
        return normalizedFile.startsWith('shared/models/') ||
               normalizedFile.startsWith('shared/database/') ||
               normalizedFile.startsWith('shared/config/') ||
               normalizedFile.startsWith('shared/utils/') ||
               normalizedFile.startsWith('shared/data/') ||
               normalizedFile.startsWith('shared/services/');
      });
