...
// Note: Discord.js EmbedBuilder is optional for dashboard context
let EmbedBuilder = null;
try {
  const discord = require('discord.js');
  EmbedBuilder = discord.EmbedBuilder;
} catch (e) {
  // Discord.js not available in dashboard context
}
...
function initializeVillage(villageName) {
    ...
    // Note: villages object not used in dashboard context
    return { health: 100, resources: 50 };
}
...
async function updateVillageHealth(villageName, healthChange) {
    ...
    const village = await Village.findOne({ name: { $regex: `^${capitalizedName}$`, $options: 'i' } });
    if (!village) {
        throw new Error(`Village "${capitalizedName}" not found`);
    }
    village.health = Math.max(0, village.health + healthChange);
    await village.save();
    return village.health;
}
...
async function damageVillage(villageName, damageAmount) {
    try {
        ...
        // Note: Discord notification code removed for dashboard context
        // Dashboard doesn't have Discord client access
...
async function applyVillageDamage(villageName, monster, thread) {
    try {
        ...
        // Note: Discord embed code removed for dashboard context
        // Dashboard doesn't have Discord client/thread access
        
        return { village: updatedVillage, removedResources };
    } catch (error) {
        handleError(error, 'villageModule.js');
