...
// Use dashboard-specific database config
const dbConfig = require('../config/database-dashboard');
...
async function connectToTinglebot() {
 try {
  ...
  tinglebotDbConnection = await mongoose.connect(uri, {
   ...
   maxIdleTimeMS: 60000,
   family: 4
   // Note: bufferCommands and bufferMaxEntries are Mongoose settings, not MongoDB connection options
   // They are set using mongoose.set() in server.js, not here
  });
 ...
 try {
  if (!inventoriesDbConnection || inventoriesDbConnection.readyState !== 1) {
   ...
   await new Promise((resolve, reject) => {
     if (inventoriesDbConnection.readyState === 1) {
       resolve();
       return;
     }
     ...
     inventoriesDbConnection.once('connected', () => {
       clearTimeout(timeout);
       resolve();
     });
     
     inventoriesDbConnection.once('error', (err) => {
       clearTimeout(timeout);
       console.error(`[db-dashboard.js]: ❌ Inventories database connection error:`, err);
       reject(err);
     });
   });
  }
  ...
  console.error(`[db-dashboard.js]: ❌ Failed to connect to inventories database:`, error.message);
  inventoriesDbConnection = null; // Reset on error
  throw error;
 }
}
...
const fetchAllCharacters = async () => {
 try {
  ...
  const CharacterModel = mongoose.models.Character || Character;
  const ModCharacterModel = mongoose.models.ModCharacter || ModCharacter;
  
  const regularCharacters = await CharacterModel.find().lean().exec();
  const modCharacters = await ModCharacterModel.find().lean().exec();
