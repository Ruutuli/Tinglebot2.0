...
// ------------------- Project Utilities -------------------
const { handleInteractionError, resetErrorCounter } = require("../utils/globalErrorHandler");
const {
 authorizeSheets,
 appendSheetData,
...
async function connectToInventories() {
 try {
  ...
  handleError(error, "db.js");
  console.error("[db.js]: ❌ Failed to connect to inventories database:", error.message);
  throw error;
 }
}
...
async function connectToVending() {
 try {
  ...
  handleError(error, "db.js");
  console.error("[db.js]: ❌ Error in connectToVending:", error.message);
  throw error;
 }
}
