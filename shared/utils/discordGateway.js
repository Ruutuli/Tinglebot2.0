...
class DiscordGateway {
  ...
  setupEventHandlers() {
    ...
    this.client.on('messageCreate', async (message) => {
      ...
      try {
        ...
        logger.error('discordGateway.js', 'Error tracking message:', error);
      }
    });
    
    // Connection events
    this.client.on('clientReady', () => {
      logger.info('discordGateway.js', `Discord Gateway connected as ${this.client.user.tag}`);
      this.isConnected = true;
    });
    
    this.client.on('error', (error) => {
      logger.error('discordGateway.js', 'Discord Gateway error:', error);
      this.isConnected = false;
    });
    
    this.client.on('disconnect', () => {
      logger.warn('discordGateway.js', 'Discord Gateway disconnected');
      this.isConnected = false;
    });
  }
  ...
  async trackMessage(message) {
    ...
    try {
      ...
      if (error.code !== 11000) {
        logger.error('discordGateway.js', 'Error saving message to database:', error);
      }
    }
  }
  ...
  async connect() {
    ...
    try {
      ...
      logger.error('discordGateway.js', 'Failed to connect to Discord Gateway:', error);
      return false;
    }
  }
  ...
  async getGuildPresences(guildId) {
    ...
    try {
      ...
      logger.error('discordGateway.js', 'Error getting guild presences:', error);
      return null;
    }
  }
  ...
  async getVoiceChannelMembers(guildId) {
    ...
    try {
      ...
      logger.error('discordGateway.js', 'Error getting voice channel members:', error);
      return 0;
    }
  }
}
