...
function logBloodMoonStatus() {
 ...
 try {
  ...
  console.error(
   `[index.js]: Error checking Blood Moon status: ${error.message}`
  );
 }
...
async function initializeClient() {
  ...
  try {
   ...
   if (interaction.isCommand()) {
    ...
    if (command && typeof command.autocomplete === "function") {
     try {
      console.log(
       `[index.js]: Using command-specific autocomplete for '${interaction.commandName}'`
      );
     ...
     console.log(
      `[index.js]: Using generic autocomplete handler for '${interaction.commandName}'`
     );
    ...
    for (const season of seasons) {
     ...
     results.push(
      [
       ...
       `✨  Special       : ${
        weather.special ? weather.special.label : "None"
       }`,
 ...
 client.on("threadCreate", async (thread) => {
  ...
  try {
   ...
   if (!starterMessage.content.replace(/\*/g, "").startsWith("Command:"))
    return;
   ...
   const images = starterMessage.attachments.map(
