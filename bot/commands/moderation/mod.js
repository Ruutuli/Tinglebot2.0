...
const bucket = require('../config/gcsService-bot');
...
async function handleTriggerRaid(interaction) {
  ...
  try {
    ...
    const { getMonstersAboveTierByRegion } = require('../../database/db-bot');
    ...
    // Pass bypassCooldownCheck=true to allow mods to trigger raids even during cooldown
    // The cooldown will still be set after the raid to prevent regular users from triggering immediately
    const result = await triggerRaid(monster, modifiedInteraction, capitalizedVillage, false, null, true);
