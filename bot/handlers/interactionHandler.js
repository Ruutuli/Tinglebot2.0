...
const { connectToInventories, connectToTinglebot } = require('@app/shared/database/db');
  ...
  client.on('messageReactionAdd', async (reaction, user) => {
    try {
      ...
      if (reaction.emoji.name === '❌') {
        ...
        const { retrieveHealingRequestFromStorage, saveHealingRequestToStorage } = require('@app/shared/utils/storage');
      ...
      if (reaction.emoji.name === '✅' || reaction.emoji.name === '☑️' || reaction.emoji.name === '✔️') {
        ...
        if (reaction.message.channel.id === '940446392789389362') { // Submissions channel ID
          ...
          if (reaction.users.cache.has(botUser.id)) {
            ...
            const { retrieveSubmissionFromStorage } = require('@app/shared/utils/storage');
