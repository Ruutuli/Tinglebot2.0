...
const { getCharacterInventoryCollection, updateCharacterById, fetchItemByName } = require('../database/db-bot.js'); 
...
async function activateJobVoucher(character, jobName, item, quantity = 1, interaction) {
    try {
        ...
        if (character.isModCharacter) {
            const { updateModCharacterById } = require('../database/db-bot.js');
