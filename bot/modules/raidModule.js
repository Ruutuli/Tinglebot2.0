...
async function processRaidBattle(character, monster, diceRoll, damageValue, adjustedRandomValue, attackSuccess, defenseSuccess, characterHeartsBefore = null) {
  try {
    ...
    if (finalDamage > 0) {
      ...
      const { fetchCharacterById } = require('../database/db-bot.js');
    ...
    if (weatherData?.special?.label === 'Blight Rain') {
      ...
      if (character.isModCharacter || character.userId === HIBIKI_USER_ID) {
        ...
        if (shouldConsumeElixir(character, 'raid', { blightRain: true })) {
          ...
          const { updateCharacterById, updateModCharacterById } = require('../database/db-bot.js');
...
async function triggerRaid(monster, interaction, villageId, isBloodMoon = false, character = null, bypassCooldownCheck = false) {
  try {
    ...
    if (!isBloodMoon) {
      ...
      // Only check cooldown if not bypassed (for mod commands)
      if (!bypassCooldownCheck) {
        ...
        console.log(`[raidModule.js]: 🔧 Cooldown check bypassed (mod command) - will still set cooldown after raid`);
      }
      
      // Update global raid cooldown (applies to all villages)
      // This happens for both regular raids and mod-triggered raids
