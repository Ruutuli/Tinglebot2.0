...
// ------------------- Quest Status Fields ------------------
const statusFields = {
    status: { type: String, enum: ['draft', 'pending', 'active', 'completed'], default: 'draft' },
...
const participantSchema = {
    ...
    completionProcessed: { type: Boolean, default: false }, // Prevents duplicate reward processing
    lastCompletionCheck: { type: Date, default: null }, // Tracks when completion was last checked
    // Art/Writing: mod approval already paid tokens (submission category); skip duplicate quest_reward at quest end
...
const additionalFields = {
    ...
    tableRollName: { type: String, default: null }, // Name of the table roll to use (legacy; first of tableRollNames when set)
...
questSchema.pre('save', function(next) {
    ...
    if (this.participants && typeof this.participants === 'object') {
        ...
        // Also: art/writing paid at mod approval may have submissionRewardTokenAmount but tokensEarned still 0.
...
/**
 * RP post requirement for a quest. `null` / unset uses default (15). `0` means no post minimum.
 * (Using || elsewhere incorrectly treated 0 as "use default 15".)
 */
function resolvePostRequirement(quest) {
    const raw = quest?.postRequirement;
    if (raw === null || raw === undefined || raw === '') {
        return DEFAULT_POST_REQUIREMENT;
    }
...
/** Parsed requiredRolls from quest doc (handles string/number from Mongo/API). */
...
/** Per-person token amount from submission flow (mod approval); used to skip duplicate quest_reward minting. */
...
/** Tokens are credited at mod approval (submission category); persist same numbers as dashboard + end-of-quest skip path. */
...
/** Parent forum/text channel Discord ID for thread slot index (1-based). */
...
questSchema.methods.checkAutoCompletion = async function(forceCheck = false, options = {}) {
    ...
    if (timeExpired) {
        ...
        // On expiry: completers → completed (same as in-period auto-complete); others still active → failed
        let failedCount = 0;
    ...
    for (const [userId, participant] of this.participants) {
        if (participant.progress === PROGRESS_STATUS.ACTIVE && meetsRequirements(participant, this, reqOpts)) {
            ...
            // Send completion notification for non-RP quests (pure RP quests handle their own notifications)
