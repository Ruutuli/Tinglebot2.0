...
// Railway has limited resources and network constraints, so we optimize accordingly
const railwayOptimizations = isRailway ? {
  // Railway-specific: Slightly smaller pool to reduce memory usage
  maxPoolSize: 15,              // Reduced from 20 for Railway memory constraints
  minPoolSize: 1,               // Reduced from 2 to save memory
  
  // Railway-specific: Faster timeouts for quicker failure detection
  serverSelectionTimeoutMS: 10000,  // 10 seconds (Railway network is reliable)
  connectTimeoutMS: 10000,          // 10 seconds
  socketTimeoutMS: 30000,           // 30 seconds (reduced from 45)
  
  // Railway-specific: More frequent health checks
  heartbeatFrequencyMS: 5000,      // 5 seconds (faster detection of issues)
  
  // Railway-specific: Aggressive idle connection cleanup
  maxIdleTimeMS: 20000,            // 20 seconds (faster cleanup to save memory)
} : {};
