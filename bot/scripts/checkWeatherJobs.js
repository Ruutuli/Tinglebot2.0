// Script to check weather reminder job status
// This helps diagnose why the weather reminder isn't running
...
async function checkWeatherJobs() {
  try {
    ...
    const weatherReminderJob = await agendaJobsCollection.findOne({
      $or: [
    ...
    logger.info('SCRIPT', `\n📋 Daily Weather Forecast Reminder Job Status:`);
    logger.info('SCRIPT', `   Job ID: ${weatherReminderJob._id}`);
    logger.info('SCRIPT', `   Name: ${weatherReminderJob.name || weatherReminderJob.attrs?.name || 'unknown'}`);
    logger.info('SCRIPT', `   Repeat Interval: ${weatherReminderJob.repeatInterval || weatherReminderJob.attrs?.repeatInterval || 'unknown'}`);
    
    const nextRun = weatherReminderJob.nextRunAt || weatherReminderJob.attrs?.nextRunAt;
    if (nextRun) {
    ...
    const lockedAt = weatherReminderJob.lockedAt || weatherReminderJob.attrs?.lockedAt;
    if (lockedAt) {
    ...
    // Check last run time
    const lastRun = weatherReminderJob.lastRunAt || weatherReminderJob.attrs?.lastRunAt;
    if (lastRun) {
    ...
    const failedAt = weatherReminderJob.failedAt || weatherReminderJob.attrs?.failedAt;
    if (failedAt) {
      logger.error('SCRIPT', `   ❌ Job has FAILED at: ${new Date(failedAt).toLocaleString()}`);
      const failCount = weatherReminderJob.failCount || weatherReminderJob.attrs?.failCount || 0;
    ...
    logger.error('SCRIPT', 'Error checking weather jobs:', error);
    console.error(error);
    process.exit(1);
  }
}
