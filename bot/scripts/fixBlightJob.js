// Script to fix the Blight Roll Call job's nextRunAt time
...
async function fixBlightJob() {
  try {
    ...
    const currentNextRun = blightJob.nextRunAt || blightJob.attrs?.nextRunAt;
    if (currentNextRun) {
    ...
    if (updateResult.modifiedCount > 0) {
      logger.success('SCRIPT', `\n✅ Successfully updated Blight Roll Call job's nextRunAt!`);
    ...
    logger.error('SCRIPT', 'Error fixing Blight job:', error);
    console.error(error);
    process.exit(1);
  }
}
